...
public class JExcelDemo extends JFrame
{
    ...
    private static final String FILTER_12 = "Excel Workbooks (*.xls) | *.xls|Excel12 Workbooks (*.xlsx) | *.xlsx";
    ...
    private JSplitPane createSubSplitPane()
    {
        ...
        _automationBarPane.setMenuButtonVisible(false);
...
//                _miSave.setEnabled(false);
        ...
        _miOpen = new JMenuItem(new AbstractAction("Open...")
        {
            public void actionPerformed(ActionEvent e)
            {
                ...
                dialog.setFilter(getFileFilter() + "|All files (*.*)|*.*");
            ...
            public void actionPerformed(ActionEvent e)
            {
                ...
                else
                {
                    ...
                    if (dialog.getSaveFileName())
                    {
                        try
                        {
                            File saveFile = new File(dialog.getFileName());
                            boolean fileExists = saveFile.exists();
                            boolean overwriteConfirmed = false;
                            if (fileExists)
                            {
                                int i = JOptionPane.showConfirmDialog(JExcelDemo.this, "Do you want to overwrite the existing file?", "Save As", JOptionPane.YES_NO_OPTION);
                                overwriteConfirmed = (i == JOptionPane.YES_OPTION);
                            }
                            if (!fileExists || overwriteConfirmed)
                            {
                                _workbook.saveAs(new File(dialog.getFileName()), getFileFormatFromExtension(saveFile.getAbsolutePath()), false);
                                _miSave.setEnabled(false);
                                if (_workbook.getFile() != null)
                                {
                                    JExcelDemo.this.setTitle(DEMO_TITLE + " - [" + _workbook.getFile().getAbsolutePath() + "]");
    ...
    static String pathText(File file, int maxLength)
    {
        ...
        if (test.length() > maxLength)
        {
            ...
            if (pos != -1)
            {
                String filename = test.substring(pos, test.length());
                if (filename.length() > maxLength)
                {
                    filename = filename.substring(0, maxLength - 3) + "...";
                }
                test = test.substring(0, pos) + filename;
                if ((maxLength - test.length() + pos) > 0)
                {
    ...
    private class JWorkbookListener extends JWorkbookEventAdapter
    {
                ...
                throws JWorkbookInterruptException
        {
            ...
            if (result)
            {
                ...
                try
                {
                    _workbook.saveAs(newFile, getFileFormatFromExtension(newFileName), true);
                }
                catch (IOException e)
                {
    ...
    private FileFormat getFileFormatFromExtension(String fileName)
    {
        FileFormat format = _workbook.getFileFormat();
        ...
        if (i != -1)
        {
            ...
            {
                format = FileFormat.OPENXMLWORKBOOK;
            }
            else
            {
                format = FileFormat.EXCEL8;
            }
        }
        return format;
    }
